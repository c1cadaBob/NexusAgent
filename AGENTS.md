...
后续开发统一使用 `main` 作为版本控制主线。除非项目负责人另行明确要求临时分支，所有任务实现、验收修复、文档更新和关键节点提交都直接发生在 `main`，并在完成任一任务或到达关键节点时提交并上传到远端仓库：
...
5. 推送 `main` 到 `origin`，并在完成报告中写明 commit hash 和 push 结果。
...
- 禁止绕过验收命令、审计记录或凭据检查直接向 `main` 推送；若项目负责人重新要求临时分支，必须先合入 `main` 后再交付。
