...
| R-004 | 防腐适配器绕过平台内核 | 极高 | P0-04 已证明 DSH 进程内 guard 可阻断 native loop 和缺少平台策略的工具调度；P1-01 已新增平台事件信封、统一状态机和跨租户拒绝单元测试，正式 adapter、防端口绕过和 sidecar 隔离仍待 P1/P2/P6 | P1-P6 |
...
| R-012 | 任务实现缺少修改前、修改过程、修改后验证审计记录 | 高 | P0-09 修改记录包纳入 P0 smoke 检查；P1-01 已按实时规划规则先填写修改前分析并补过程/验证记录，集中台账关闭项必须引用关闭任务/commit，阶段门禁仍需持续拒绝空白审计记录 | P0-P8 |
