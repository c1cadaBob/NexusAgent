...
| R-010 | AI 自动生成排期时遗漏只读目录、上游不可见、防绕过或验收命令等高危约束 | 高 | P0-08 已把阶段门禁命令、MVP/P7/P8 边界和重排触发器写入排期基线；P0-09 仍需把这些约束固化到自动填充提示词 | P0-P8 |
| R-011 | 任务提示词文档与实施规划任务表不同步 | 中 | P0-08 已同步需求矩阵和 README 的排期基线引用；后续 P0-09/P0-10 仍需检查任务提示词同步 | P0-P8 |
| R-012 | 任务实现缺少修改前、修改过程、修改后验证审计记录 | 高 | P0-08 修改记录包纳入 P0 smoke 检查；阶段门禁仍需持续拒绝空白审计记录 | P0-P8 |
