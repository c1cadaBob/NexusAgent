...
- 遗留风险：P8-02 只发布当前仓库真实 runtime 支撑的 `platform-api` 与 `web-console` GHCR candidate images；内部 service 镜像、生产后端、备份恢复、完整 sidecar/OS 隔离和法务 NOTICE 发布包仍由 P8-03/P8-04 承接。
...
- 遗留风险：P8-03 是模板、静态验证和 deterministic local drill，不代表真实客户基础设施 rollout、外部 provider sidecar/OS 生产隔离、法务 NOTICE 发布包或真实生产备份恢复演练；这些继续由 P8-04 或后续发布治理承接。
