...
| R-008 | 外部基础设施选型过早锁死导致 P1/P8 返工 | 高 | P0-07 已把 Temporal/OPA/NATS/Kafka/MinIO/S3/Vault/KMS/Keycloak/OpenTelemetry/Grafana/pgvector/Qdrant 标记为候选或可借鉴项目；P1-01/P1-03 仅固定平台抽象、schema、状态机、Clock、内存 Event Bus 和 adapter lifecycle，不锁死生产消息、对象存储、密钥或观测后端，生产选型仍待 P1/P8 | P0/P1/P8 |
...
| R-012 | 任务实现缺少修改前、修改过程、修改后验证审计记录 | 高 | P0-09 修改记录包纳入 P0 smoke 检查；P1-01/P1-02/P1-03 已按实时规划规则先填写修改前分析并补过程/验证记录，集中台账关闭项必须引用关闭任务/commit，阶段门禁仍需持续拒绝空白审计记录 | P0-P8 |
