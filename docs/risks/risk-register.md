...
| R-005 | 记忆快照、并发写入和脏数据 | 高 | P0-03 已验证实验模式拒绝原生文件记忆直读直写；P1-04 已新增本地 Memory Gateway、五层 memory scope、版本和租户隔离测试；P3-02 已新增三层 planner snapshot sanitizer、`expected_version` 冲突检测、跨用户/租户过滤、vendor proxy fail-closed 和非 planner-only drift 回归测试；P7-02/P7-04 已补 retention、soft delete 和 conflict queue；P8-03 已锁定 Memory Store 默认 PostgreSQL + pgvector，并通过 metadata-only restore drill 验证 tenant version continuity、conflict metadata 和 memory text 不导出 | P1/P3/P6/P8 |
...
| R-008 | 外部基础设施选型过早锁死导致 P1/P8 返工 | 高 | P0-07 已把 Temporal/OPA/NATS/Kafka/MinIO/S3/Vault/KMS/Keycloak/OpenTelemetry/Grafana/pgvector/Qdrant 标记为候选或可借鉴项目；P1-01/P1-03/P1-04 仅固定平台抽象、schema、状态机、Clock、内存 Event Bus、本地 Artifact/Memory/Credential；P1-05 仅固定本地 RBAC、hash-chain Audit 和 Observability 接口；P1-06 仅固定开发 Compose、Node health 占位、端口和 smoke；P8-01 已关闭 `OQ-DEPLOY-001`；P8-03 已按用户锁定默认关闭 `OQ-INFRA-002/003/004/005` 与 `OQ-MEMORY-002`，生产 Alpha 后端为 NATS JetStream、S3-compatible Object Store、Vault、PostgreSQL + pgvector 和 OpenTelemetry + Prometheus/Loki/Tempo，企业替换必须保持同一平台契约与 restore gate | P0/P1/P8 |
...
| R-014 | 第三方插件凭据、artifact 或日志泄漏 | 极高 | P1-04 已新增本地 Credential Center 引用/短租约元数据、Artifact Store metadata-only 事件、credential material 不进入 reference/audit/event 的安全测试；P2-03 已验证 DSH executor stdout/stderr/artifact candidates 入库前脱敏，artifact/event 只暴露 metadata；P3-04 已验证 Hermes/OpenClaw Plugin Bridge 拒绝 secret/native/provider markers；P5 公共 API/控制台/SDK/docs 不回显 secret/native/provider 字段；P6 安全矩阵与恶意插件隔离验证 bypass fail closed；P8-03 已新增 credential reference-only restore、artifact SHA-256 restore、backup secret isolation 和 metadata-only drill，确认恢复报告不含 memory text、credential material、native URL/path/session/error、provider runtime、external locator 或本地绝对路径；P8-04 legal/delivery docs 和 targeted security tests 继续验证交付包、NOTICE 和回滚手册不含 secret、native locator 或 public surface 泄漏；完整第三方插件生产 sidecar/OS 隔离仍待后续治理 | P3-P8 |
| R-015 | 第三方插件许可证、NOTICE 或再分发条款不清 | 高 | P0-05 已确认三大上游根许可证为 MIT；P5-01 已要求管理员插件导入提供 `expected_sha256`、`license`、`notice_status`、`risk_level` 和版本元数据，并用 P5 security tests 验证缺失或含 URL/path/credential/manifest 绕过字段的导入失败；P5-04 SDK examples 和开发者文档继续展示 hash/license/notice_status/risk_level 元数据入口，并用 SDK/docs leakage tests 验证公共文档不携带原生来源或 secret material；P8-02 已把 provider/plugin hash、license、notice_status、risk、allowlist 和 rollback target 纳入 release gate；P8-04 已新增 `config/legal-notice.p8.json`、`docs/legal/THIRD_PARTY_NOTICE.md`、legal validator 和 P8 smoke marker，按 P8 Alpha 仓库级发布包关闭 `OQ-LEGAL-001`；外部律师意见或客户合同审查仍是发布治理外部活动 | P0/P5/P8 |
| R-016 | 插件更新破坏 OpenClaw/Hermes/DSH provider 兼容性 | 高 | P2-01 已为 DSH 建立 provider registry、默认 provider、启用/禁用/回滚和 targeted fixture；P2-02 已证明默认 provider 与候选 fixture provider 可复用同一平台 execution contract；P2-03 已把 provider raw output 约束在 `ArtifactReference`/Event Bus/平台错误码归一化边界内；P2-04 已用 canary fixture 验证失败 provider、disabled provider 和 rollback 恢复路径；P3-01 已为 Hermes 建立 provider registry、默认 provider、启用/禁用/回滚和 P3 smoke；P3-03 已新增 P3 ExecutionPlan schema、validator 和 provider fixture 复用，保留 P0 marker 仅作历史证据；P4-01 已为 OpenClaw 建立 provider registry、默认 provider、启用/禁用/回滚和 P4 smoke；P8-02 已新增 provider/plugin compatibility matrix、GHCR candidate release gate、canary-first promotion、release pause 和 rollback target 校验；P8-04 已新增 provider/plugin rollback manual、delivery readiness gate 和 public API stability tests，确认替换/禁用/回滚不改变平台 API；真实生产 sidecar/OS 隔离仍待后续治理 | P2-P4/P8 |
...
- R-013/R-014/R-016：P8-01 仅允许 DSH/Hermes/OpenClaw adapters 作为内部 workload 存在，不暴露 provider-native gateway/tool/agent/plugin/session/runtime/credential/public port；provider 升级兼容矩阵由 P8-02 承接，许可证发布包和交付手册由 P8-04 承接，真实插件运行时、生产 SecretBackend 和 OS/sidecar 隔离仍由后续治理承接。

## P8-02 CI/CD、发布和上游追踪更新

- R-001/R-016：P8-02 新增 `config/provider-compatibility.p8.json`、`config/plugin-compatibility.p8.json` 和 `config/release-gate.p8.json`，把 DSH/Hermes/OpenClaw current default、vendor tree sha、required tests、`rollback_target`、`canary_first` 和 release pause 纳入机器可读门禁。
- R-010/R-012：`.github/workflows/p8-release-gate.yml` 和 `tests/smoke/P8.sh` 已检查规划生成、diff、P0-P8 smoke、targeted release/security tests、P8-02 审计无占位、Date.now 禁用、secret/build artifact 扫描和 release/upstream validators，失败时不得发布。
- R-013/R-014/R-015：插件升级门禁要求 hash、license、notice_status、risk_level、allowlist_status、required tests 和 rollback target；租户不得自助安装第三方插件，缺元数据或 breaking change 触发 `P8-02_PLUGIN_UPGRADE_GATE_PAUSE`。
- R-008/R-016：`weekly-upstream-check.mjs` 默认不依赖网络，配置远端后可选检查；当前 upstream remote/commit 未确认时输出 `UPSTREAM_IDENTITY_UNCONFIRMED` 并阻止 production default promotion，避免编造上游来源。
- 遗留风险：P8-02 只发布当前仓库真实 runtime 支撑的 `platform-api` 与 `web-console` GHCR candidate images；内部 service 镜像、生产后端、备份恢复由 P8-03 承接，法务 NOTICE 发布包和交付手册由 P8-04 承接，完整 sidecar/OS 隔离仍由后续治理承接。

## P8-03 告警、备份和恢复更新

- R-005/R-008：P8-03 将 Memory Store 默认关闭为 PostgreSQL + pgvector，把 Event Bus、Artifact Store、Credential Center 和 Observability 默认关闭为 NATS JetStream、S3-compatible Object Store、Vault 与 OpenTelemetry + Prometheus/Loki/Tempo，同时要求企业替换保持相同平台契约、RPO `15m`、RTO `4h` 和 restore acceptance gates。
- R-010/R-012：`tests/smoke/P8.sh` 已扩展 P8-03 required files、审计无占位、observability/backup schema markers、RPO/RTO/backend defaults、Date.now 禁用、metadata-only restore report 扫描和 targeted deployment/security tests，降低生产门禁遗漏风险。
- R-014：`platform/backup-restore/index.ts` 与 `tests/security/p8-backup-secret-isolation.test.mjs` 验证 backup/restore 报告仅导出 event/audit/artifact/memory/credential metadata；credential material、memory text、native/provider/local locator 和真实外部网络字段 fail closed。
- 遗留风险：P8-03 是模板、静态验证和 deterministic local drill，不代表真实客户基础设施 rollout、外部 provider sidecar/OS 生产隔离、外部律师意见或真实生产备份恢复演练；P8-04 承接法务 NOTICE 发布包和交付手册，其他事项继续由后续发布治理承接。

## P8-04 交付文档、法务 NOTICE 和阶段门禁更新

- R-010/R-012：P8-04 新增 `config/delivery-readiness.p8.json`、交付手册、P8 阶段门禁报告、validators 和 P8 smoke markers，防止交付文档、审计记录或阶段门禁遗漏。
- R-014/R-015：P8-04 新增 `config/legal-notice.p8.json` 和 `docs/legal/THIRD_PARTY_NOTICE.md`，验证 provider 根许可证、nested notice evidence、plugin license/notice/hash/risk/allowlist/rollback 证据齐全，并按 P8 Alpha 仓库级发布包关闭 `OQ-LEGAL-001`。
- R-016/R-007：P8-04 新增 provider/plugin rollback manual 和 public API stability tests，确认 provider/plugin 替换、禁用或回滚只改变内部兼容矩阵和治理状态，不改变 `/v1/*` API、SDK 方法或控制台路由。
- 遗留风险：真实 upstream remote/commit 仍由 `OQ-UPSTREAM-001/002/003` release pause 管控；真实客户基础设施 rollout、外部律师意见、完整 provider sidecar/OS 隔离和客户合同审查不在 P8-04 仓库门禁内伪造完成。
...
- 遗留风险：P7-01 不关闭 P7-02 主动遗忘、P7-03 技能自动评测、P7-04 Token 预算计费维度、P7-05 定时长期目标任务或 P8 发布治理；如后续将质量信号暴露给 API/控制台，必须另建任务并补公共面泄漏门禁。

## P7-02 主动遗忘与保留策略更新

- R-003/R-005/R-007：P7-02 使用 soft delete 和 metadata-only sweep/delete result，`tests/security/p7-memory-retention-leakage.test.mjs` 验证 memory text、raw credential、native URL/path/session/error 和 provider runtime 不进入响应、事件、metrics/logs 或控制台 view-model。
- R-006/R-009：P7-02 采用 manual sweep 而非后台 scheduler，默认 conservative 策略只自动处理 `session` 层，控制 P7 高级能力对 MVP 资源和运行时复杂度的影响。
...
- 遗留风险：P7-03 不关闭 P7-04 Token 预算计费维度、P7-05 定时长期目标任务、真实业务评测平台、真实插件运行时、生产 sidecar/OS 隔离、插件升级兼容矩阵或 P8 发布治理。
...
- 遗留风险：P7-04 不关闭 P7-05 定时长期目标任务、durable billing、真实模型 token 计量、生产 quota backend、durable Memory Gateway 存储/检索、真实业务评测平台或 P8 发布治理。

## P7-05 定时长期目标任务更新

- R-003/R-007：P7-05 scheduled goal API、SDK、Console view-model、docs catalog、events、audit 和 logs 只暴露平台字段；`tests/security/p7-scheduled-goals-leakage.test.mjs` 验证 raw credential、native URL/path/session/error、provider runtime/binding、真实网络 URL 和本地路径无法进入公共投影。
- R-006/R-009：定时长期目标任务采用 Default Off + manual tick，不运行后台 daemon；每租户 active goal、每次 due scan 和单 goal budget units 均受 alpha in-memory resource budget 约束，降低 P7 调度能力对 MVP 容量和 Token 风险的影响。
- R-010/R-012：`tests/smoke/P7.sh` 增加 P7-05 required files、审计无占位、scheduled goal schema/default-off/cron/manual tick/API/SDK/Console markers、Date.now 禁用扫描、public leakage scan 和 targeted unit/integration/security/contract tests。
