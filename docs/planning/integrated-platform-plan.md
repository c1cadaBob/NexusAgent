...
阶段完成必须同时满足：任务清单全部有结果；代码/文档产物存在；质量门禁通过；阶段冒烟脚本输出 PASS；依赖阶段的验收证据已归档；已回扫当前阶段及其之前阶段的所有待确认问题、任务修改记录包、风险登记册、需求追踪矩阵和专业文档；待确认问题已关闭，或已明确责任人、截止时间、后续实时规划提示词和不阻塞理由；风险登记册已更新；对应任务 ID 文档中的“修改记录包”已按修改前、修改过程、修改后验证三段补齐。
