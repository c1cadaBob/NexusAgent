...
Hermes 原生 API、`MEMORY.md`/`USER.md` 文件机制和自然语言回复全部由平台封装。

| 任务ID | 任务名称 | 所属阶段 | 涉及文件开发路径 | 修改说明 | 输入 | 输出 | 验收条件 | 预估人天 | 前置依赖 | 潜在卡点 |
|---|---|---|---|---|---|---|---|---:|---|---|
| P3-01 | Hermes planner-only 改造 | P3 | `/opt/project/NexusAgent/vendor/hermes-agent-main/agent/conversation_loop.py`、`agent/tool_executor.py`、`hermes_cli/loops.py`、`hermes_cli/gateway.py` | 依据 P0 调用图关闭原生网关/工具 Runtime；只保留规划所需调用；所有补丁记录在 vendor 清单 | P0-03 证据 | Hermes planner patch、回归测试 | 规划请求只返回平台 `ExecutionPlan`；禁止原生网关启动和工具直接执行 | 6 | P0-03、P1 | Hermes 启动路径和插件依赖复杂 |
| P3-02 | 修复 memory_tool 快照缺陷并代理化 | P3 | `/opt/project/NexusAgent/vendor/hermes-agent-main/tools/memory_tool.py`、`agent/memory_manager.py`、`agent/memory_provider.py`；`platform/adapters/hermes/`、`platform/memory-gateway/` | 先复现快照/并发写入问题；修复必须保留 drift/read-failure 防护；读写改为平台代理 | Hermes memory 实测、P1 Memory Gateway | Hermes patch、Memory Gateway adapter、数据迁移/恢复说明 | 读写全部通过平台 API；文件外部直接修改可检测；快照不把未授权内容注入 planner 输入 | 7 | P1-04、P3-01 | 五层记忆策略未最终确定 |
| P3-03 | 标准化 ExecutionPlan | P3 | `platform/contracts/execution-plan.schema.json`、`platform/adapters/hermes/` | 将 Hermes 输出转换为可校验的步骤、工具意图、预算、依赖和风险字段；禁止自然语言最终回复 | P1-01、P3-01 | schema、转换器、样例 | schema 校验失败时拒绝进入 Coordinator；不存在上游类型泄漏 | 4 | P3-01 | Hermes 输出可能包含非结构化文本 |
| P3-04 | Hermes 集成和防直读验证 | P3 | `tests/integration/hermes-adapter.*`、`tests/security/hermes-memory-bypass.*`、`tests/smoke/P3.sh` | 验证只能通过 Memory Gateway 访问记忆，不能从平台 API 读取 Hermes 文件 | P3-01 至 P3-03 | 集成、隔离和冒烟测试 | 直接文件路径、原生端口和伪造 adapter 身份均失败；平台记忆 API PASS | 3 | P3-03 | 容器卷权限和进程身份配置 |
...
OpenClaw 只负责多渠道消息接收、标准 TaskRequest 转换和结果出站适配。

| 任务ID | 任务名称 | 所属阶段 | 涉及文件开发路径 | 修改说明 | 输入 | 输出 | 验收条件 | 预估人天 | 前置依赖 | 潜在卡点 |
|---|---|---|---|---|---|---|---|---:|---|---|
| P4-01 | OpenClaw gateway-only 改造 | P4 | `/opt/project/NexusAgent/vendor/openclaw-main/src/gateway/agent-turn/agent-request-routing.ts`、`agent-run-dispatch.ts`、`agent-run-execution-phase.ts`、`agent-turn-service.ts`、`src/channels/inbound-event/envelope.ts` | 按 P0 调用图隔离原生 Agent、本地工具和独立记忆；保留渠道 transport 所需入口 | P0-02 证据 | OpenClaw patch、渠道回归测试 | gateway 只产生标准消息事件；不能在无 Coordinator 授权时启动原生 Agent | 6 | P0-02、P1 | 渠道和 Agent 逻辑耦合 |
| P4-02 | OpenClaw 防腐适配器 | P4 | `platform/adapters/openclaw/` | 接收 IM/WebSocket 消息，转换为 TaskRequest；接收平台结果后做渠道输出；不暴露 OpenClaw 类型 | P1-03、P4-01 | 渠道 adapter、映射表、契约测试 | 所有入站事件带租户/会话/trace 信息；出站只发送平台结果 | 5 | P4-01 | 多渠道 payload 差异和重试 |
...
| P4-04 | 渠道防绕过和冒烟 | P4 | `tests/security/openclaw-bypass.*`、`tests/integration/channel-routing.*`、`tests/smoke/P4.sh` | 验证所有渠道消息经过 Policy-Gate 和 Coordinator | P4-01 至 P4-03 | 拦截测试、渠道集成、冒烟脚本 | 直接触发原生 Agent、伪造内部 header 和未知渠道均失败 | 3 | P4-03 | 渠道厂商网络/凭据不可用 |
...
P5 只有在 P1-P4 全部验收通过后才能开始；产品层不得依赖任何底层原生概念。

| 任务ID | 任务名称 | 所属阶段 | 涉及文件开发路径 | 修改说明 | 输入 | 输出 | 验收条件 | 预估人天 | 前置依赖 | 潜在卡点 |
|---|---|---|---|---|---|---|---|---:|---|---|
| P5-01 | 统一 REST/gRPC API | P5 | `product/api/`、`platform/contracts/`、`docs/contracts/openapi.yaml` | 实现任务提交/查询/取消、技能、记忆、租户、用户、权限、审批和预算 API | P0 OpenAPI、P1-P4 adapters | API 服务、OpenAPI、错误码和契约测试 | OpenAPI 契约测试全通过；响应不含 Hermes/OpenClaw/DSH 字段；RBAC 和租户隔离生效 | 10 | P1-P4 | REST/gRPC 同期交付范围待确认 |
| P5-02 | Web 管理控制台 | P5 | `product/web-console/` | 实现租户、Agent 实例、任务面板、审计、监控、技能和审批视图 | P5-01 | React/Vite 控制台 | 关键操作均调用平台 API；无底层品牌/原生路径；权限不足时界面和 API 同时拒绝 | 12 | P5-01 | 交互/视觉规范和实时刷新策略 |
...
| P5-04 | SDK 和开发者文档 | P5 | `product/sdk/`、`product/docs-site/`、`docs/contracts/` | 提供平台 SDK 示例、认证、任务、事件、错误和 webhook 文档 | P5-01 | SDK、示例、开发者手册 | 示例可运行；只引用平台 API；契约测试覆盖文档示例 | 5 | P5-01 | SDK 语言范围待确认 |
...
|---|---|---:|---|---|---|---|
| P0 | 快照、三种剥离实验、接口摸底、OpenAPI、服务蓝图、排期基线 | 22 | 无 | P0 可行性报告、vendor、OpenAPI 初稿、十个基础服务蓝图、开发排期 | 极高 | 否 |
...
阶段完成必须同时满足：任务清单全部有结果；代码/文档产物存在；质量门禁通过；阶段冒烟脚本输出 PASS；依赖阶段的验收证据已归档；待确认问题已关闭或明确责任人/截止时间；风险登记册已更新。

### 阻塞条件

出现以下情况不得标记完成：核心接口没有可重复验收命令；绕过平台的负向测试通过；任一关键 ID 或事件无法追踪；生产配置仍启用热更新/调试端口；测试失败被跳过；上游行为仅凭推测记录。

连续三个工作日无法解决、且没有替代实现或回退路径的问题，必须提交架构负责人进行降级/止损决策；P0 级阻塞直接暂停后续阶段。

## 一页管理层极简摘要

目标：交付一个对外独立的 AI Agent 平台，统一 API、控制台、渠道、任务、技能、记忆、租户和审计；Hermes、OpenClaw、DSH 仅作为内部实现依赖。十个基础服务的功能和整合边界已在服务蓝图中单独维护，开发日历和资源假设已在排期基线中维护。
...
MVP 时间窗口：按 199 人天工程估算，P0-P6 为首个可用平台基线；当前排期从 2026-08-24 启动，MVP 候选在 2026-11-27 冻结，生产交付候选在 2026-12-24 冻结。实际日历时间取决于团队规模、上游变更、渠道凭据和待确认问题关闭时间。若 Hermes 剥离失败，沿轻量化 OpenClaw + DSH 路线保留平台外壳交付。
