...
P0 阶段允许进入后续阶段。P0-01 至 P0-11 已在 `main` 上完成并纳入 `tests/smoke/P0.sh`，本次门禁关闭 P0 到期的 4 个 `OQ-*`，P7-04 新增 `OQ-BUDGET-001` 后其余 20 个问题继续保持 `自动确认`，按各自最晚确认阶段进入 P1-P8 后续任务。
...
| P0-09 | 通过 | P7-04 新增 `OQ-BUDGET-001` 后，24 个 OQ 的集中台账和确认文件目录已建立。 |
...
以下 20 个问题仍为 `自动确认`，但不阻塞 P0 门禁，因为它们的最晚确认阶段在 P1 或之后，且默认解决方案已经在对应确认文件中登记：
...
- 当前阶段及之前阶段不存在 `打开` 问题。
- P0 到期问题已关闭并指向解决说明文档。
- 仍为 `自动确认` 的 20 个问题已按后续阶段保留，不阻塞 P0 进入后续阶段。
