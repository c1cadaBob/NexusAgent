...
| 阶段历史问题回扫 | 每个阶段结束前必须确认当前阶段及其之前阶段是否仍有未处理、未同步或需要修复的问题 | 阶段门禁必须列出历史 `OQ-*` 状态、修复结果、后续实时规划提示词和是否阻塞下一阶段 |
...
实时规划提示词必须把待确认问题处理放在实际实现之前，执行顺序固定如下：

1. 开始实现任务前，先填写对应任务 ID 文档的“修改前分析”。
2. 在“修改前分析”中核对 `docs/planning/open-questions-register.md` 和 `docs/planning/open-questions/`；如果存在 `打开`、缺少确认文件、或已 `自动确认` 但尚未同步修复的问题，必须先处理该问题，再进入下一步实现。
3. 修改过程中持续补充“修改过程记录”，不得等到最后一次性补写。
4. 完成验证后补齐“修改后验证与总结”，并说明待确认问题是已关闭、已同步、还是已重新排入后续任务。
5. 如果仍存在未处理的待确认问题，则在确认后根据当前阶段进度排到后续任务，并在 `docs/planning/task-prompts/` 相应阶段文件夹创建或更新实时规划提示词。
6. 已 `自动确认` 但尚未同步到任务文档、风险、追踪矩阵或专业文档的问题，必须创建同步修复类实时规划提示词；例如 P0-01 到 P0-08 的遗留待确认问题由 P0 后续同步修复任务承接。
...
- 依赖阶段验收证据缺失，不能通过。
- 阶段结束前未回扫当前阶段及其之前阶段的历史 `OQ-*`、任务修改记录包、风险登记册和需求追踪矩阵，不能通过。
- 发现 `打开`、缺少确认文件、`自动确认` 但未同步修复、`人工确认` 但未关闭，或历史专业文档仍未同步的问题，必须先修复或创建后续实时规划提示词；没有责任人、截止阶段和不阻塞理由时，不能进入下一阶段。
...
5. 当前阶段及之前阶段历史问题回扫结果：已修复、已同步、已排入后续实时规划提示词、或仍阻塞。
6. 允许进入下一阶段时的限制条件。
7. 不允许进入下一阶段时的补救排期。
