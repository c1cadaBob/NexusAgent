...
> 文档状态：P2-04 更新。本文只定义 DeepSeek Harness（DSH）作为内部执行后端的兼容、升级、回滚和替换边界；不确认任何未实测的 DSH 上游行为。
...
- `node --test tests/integration/dsh-adapter-failover.test.mjs tests/security/dsh-bypass.test.mjs tests/security/dsh-network-isolation.test.mjs` 验证 P2-04 防绕过、静态端口隔离和 provider failover/rollback。
- `bash tests/smoke/P2.sh` 汇总 P2-01/P2-04 静态、registry、adapter contract/security/integration、sandbox/artifact/event、防绕过、failover、vendor targeted tests、manifest 登记和公共泄漏检查。
