...
- [P8-04 管理员交付手册](operations/admin-handoff.md)：P8-04 管理员交付包，覆盖 platform admin/tenant admin 操作边界、插件治理、发布检查、`OQ-LEGAL-001` closure 和上游身份 release pause。
- [P8-04 开发者交付手册](operations/developer-handoff.md)：P8-04 开发者交付包，覆盖 `P8-04_PUBLIC_API_STABILITY`、OpenAPI contract、TypeScript SDK examples 和不新增公共产品接口的验证命令。
...
- [P7 计划质量信号](../platform/coordinator/plan-quality.ts)：P7-01 基线，覆盖默认关闭的 `nexus.plan_quality.p7.v1` 内部质量信号、ExecutionPlan deterministic static scoring、内部 Observability metrics/logs、资源预算和泄漏清洗。
- [P7 主动遗忘与保留策略](../platform/memory-gateway/index.ts)：P7-02 基线，覆盖默认启用的 `nexus.memory_retention.p7.v1` Conservative retention、Soft delete、Manual sweep、管理员 API+Console 操作和 metadata-only 审计。
...
- [P8 CI/CD、发布和上游追踪](operations/release-gate.md)：P8-02 基线，覆盖 `.github/workflows/p8-release-gate.yml`、`config/release-gate.p8.json`、provider/plugin compatibility matrix、GHCR tag candidate publish、canary-first promotion、release pause、上游 optional remote check、真实 runtime image scope 和 P8-02 targeted release/security tests；生产后端和备份恢复由 P8-03 承接，法务 NOTICE 包和交付手册由 P8-04 承接，完整 sidecar/OS 隔离继续由后续治理承接。
- [P8 告警、备份和恢复](operations/backup-restore.md)：P8-03 基线，覆盖 `config/observability-alerts.p8.json`、`config/backup-restore.p8.json`、`platform/observability/readiness.ts`、`platform/backup-restore/index.ts`、RPO `15m`、RTO `4h`、NATS JetStream、S3-compatible Object Store、Vault、PostgreSQL + pgvector、OpenTelemetry + Prometheus/Loki/Tempo、metadata-only restore drill、P8-03 targeted deployment/security tests 和 `OQ-INFRA-002/003/004/005`、`OQ-MEMORY-002` 已关闭结论；法务 NOTICE 包和交付手册由 P8-04 承接，真实客户 rollout 和完整 sidecar/OS 隔离继续由后续治理承接。
- [P8 交付文档与 legal NOTICE](operations/delivery-readiness.md)：P8-04 基线，覆盖 `config/delivery-readiness.p8.json`、`config/legal-notice.p8.json`、管理员/开发者 handoff、upgrade/migration、provider/plugin rollback、`THIRD_PARTY_NOTICE`、P8 阶段门禁报告、P8-04 targeted tests、`P8-04_PUBLIC_API_STABILITY` 和 `OQ-LEGAL-001` 已关闭结论；真实 upstream remote/commit、外部律师意见、客户 rollout 和完整 sidecar/OS 隔离继续由后续治理承接。
...
- P7-01 已新增默认关闭的元认知与计划质量信号：`platform/coordinator/plan-quality.ts` 只评估 `ExecutionPlan`，显式开启后写入内部 Observability metrics/logs/timeline，关闭或评估异常不影响 planner dispatch；`tests/smoke/P7.sh` 纳入 P7-01 required files、审计无占位、默认关闭、内部 Observability、无公共 API/SDK/控制台改动、Date.now 禁用和 targeted unit/integration/security tests。P7-02/P7-03/P7-04/P7-05 已作为用户指定的 P7 增量继续落地，真实业务评测平台和 P8 发布治理继续后续关闭。
- P7-02 已新增主动遗忘与保留策略：`platform/memory-gateway/index.ts` 提供默认启用的 Conservative retention、Soft delete、Manual sweep、metadata-only Event Bus/Observability 证据；`product/api`、OpenAPI、TypeScript SDK 和 Web Console 增加管理员 memory retention 操作，tenant/platform admin 可管理，operator/viewer fail closed。`tests/smoke/P7.sh` 纳入 P7-02 required files、API/SDK/console markers、泄漏扫描和 targeted tests。P7-03 已继续新增技能自动评测回归，P7-04 已补 Token 预算计费维度和记忆冲突管理员队列，P7-05 已补定时长期目标任务，P8 生产存储/发布治理仍由后续任务关闭。
...
- P8-01 已新增生产 Compose/Kubernetes 编排：`deploy/docker-compose.prod.yml` 覆盖十个生产服务，只有 `platform-api` 与 `web-console` 发布宿主端口；`deploy/k8s/` 覆盖 namespace、service accounts、ConfigMap、Secret template、deployments、services、Ingress、NetworkPolicy 和 kustomization，所有 workload 具备 probes、resource requests/limits、non-root、read-only root filesystem、no privilege escalation 和 capabilities drop all；`tests/smoke/P8.sh` 纳入生产隔离、OQ-DEPLOY-001 已关闭、Date.now 禁用、secret/build artifact 扫描和 targeted deployment/security tests。P8-02 已补 CI/CD 发布和上游追踪，P8-03 已补生产后端、告警、备份恢复，P8-04 已补交付手册、legal NOTICE、P8 门禁和 `OQ-LEGAL-001` closure；真实 upstream remote/commit、客户 rollout 和完整 sidecar/OS 隔离继续由后续治理承接。
