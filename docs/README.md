...
- P5-01 已完成 REST-first 平台 API Alpha：`product/api/index.ts` 暴露 health、tasks、skills、capabilities、memory、tenants/users/permissions、approvals、budget 和 admin plugin governance；`platform/plugin-governance/index.ts` 将既有 Plugin Bridge 证据投影为平台能力/插件清单，公共 API 不返回 source_ref、provider binding、runtime、session、URL/path 或 secret material；`tests/smoke/P5.sh` 已纳入 P5-01 OpenAPI contract、REST integration、public leakage 和 plugin governance security tests。gRPC/protobuf、streaming、生产 IdP/SSO、SDK 语言和控制台体验继续由 P5-02/P5-04/P8 承接。
