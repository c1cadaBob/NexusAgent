...
import { actionEnabled, assertConsolePublicValue, buildConsoleDashboardModel, projectPluginRow, visibleNavigation } from '../../product/web-console/src/viewModel.ts';

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const blocked = /Hermes|OpenClaw|DeepSeek|\bDSH\b|native_|raw_credential|credential_material|provider_(?:agent|task|cancel|binding)|source_ref|session_id|file_path|memory_path|tool_name|\/(?:opt|tmp|var|etc|home|usr)\//i;
...
test('web console permission view hides plugin governance for non-platform admins', () => {
  ...
  assert.ok(admin && tenantAdmin && viewer);
